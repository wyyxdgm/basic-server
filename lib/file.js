...
function walkSync(_path, floor, fn) {
    handleFileSync(_path, floor, fn);
    floor++;
...
module.handleFileSync = handleFileSync;
...
function writeIfChange(__path, content) {
    if (!fs.existsSync(__path)) return fs.writeFileSync(__path, content);
    var tmp = fs.readFileSync(__path, 'utf-8').toString();
    if (tmp != content) return fs.writeFileSync(__path, content);
    return true;
}

module.exports.writeIfChange = writeIfChange;
