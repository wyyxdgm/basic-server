...
function _post(protocol, options, content, fn, timeoutFunc) {
    var req;
    if (protocol == "http:") {
        ...
        req.setTimeout(5000, timeoutFunc || function() {
            fn('连接超时!')
        });
...
function postForm(urlStr, json, fn, timeoutFunc) {
    var urlParsed = url.parse(urlStr);
    var content = querystring.stringify(json);
    //	Logger.debug(content);
    var options = {
    ...
    if (urlParsed.port) options.port = urlParsed.port;
    _post(urlParsed.protocol, options, content, fn, timeoutFunc);
}
